<script setup lang="ts">
import { features5 } from '/@src/data/blocks/features'
import {
  featureBlockG,
  featureBlockGProps,
  featureBlockGSample,
} from '/@src/data/docs/blocks/feature/blockG'
</script>

<template>
  <Section id="block-g-demo" bordered-bottom>
    <Container>
      <DemoTitle
        title="Block G"
        subtitle="Feature block G variation 1"
        link="block-g-props"
      />

      <FeatureBlockG
        :features="features5"
        image="/assets/illustrations/features/feature-image-meditation.svg"
        dark-image="/assets/illustrations/features/feature-image-meditation.svg"
      />
    </Container>
  </Section>

  <Section color="darker">
    <Container>
      <DemoTitle
        title="Block G"
        subtitle="Feature block G variation 2"
        link="block-g-props"
        inverted
      />

      <FeatureBlockG
        :features="features5"
        image="data:image/gif;base64,replace_with_your_image"
        dark-image="data:image/gif;base64,replace_with_your_image"
        links
        inverted
      />
    </Container>
  </Section>

  <Section bordered-bottom>
    <Container>
      <DemoTitle
        title="Block G"
        subtitle="Feature block G variation 3"
        link="block-g-props"
      />

      <FeatureBlockG
        :features="features5"
        image="/assets/illustrations/features/feature-image-tablet.svg"
        dark-image="/assets/illustrations/features/feature-image-tablet.svg"
        links
      />
    </Container>
  </Section>

  <Section id="block-g-props" bordered-bottom>
    <Container>
      <DemoTitle title="Block G Props" subtitle="Available props for block G" />

      <div class="py-4"></div>

      <DemoTabs>
        <template #code>
          <PrismCode language="html" :code="featureBlockG" />
        </template>
        <template #props>
          <DemoProps :props="featureBlockGProps.props" />
        </template>
        <template #sample>
          <PrismCode language="javascript" :code="featureBlockGSample" />
        </template>
        <template #usage>
          <p class="paragraph rem-95">
            Vulk ships with highly customizable and versatile Feature blocks
            that you can use in any page. Each block comes with it's own styles
            and props. Use the props to build your own examples or use one of
            the prebuilt examples. Every block is natively supporting dark mode.
            However, there are some cases when even in light mode, the section
            background is dark or colored. In that case, you can use the
            <code>inverted</code>
            prop to invert the component when in light mode. See markup and
            props tab for more details about usage.
          </p>
        </template>
      </DemoTabs>
    </Container>
  </Section>
</template>
